...
Options:
    ...
    --svsfile is the choice to judge whether you have a file of sv,only two choice  [default: nosvsfile]
    --min_ratio is the threshold ratio to find any missing blocks in matrix that has more than threshold of the whold matrix [default: 0.2]
    --fsvs is the bed file which has four columns as :chromosome,breakpoint1,breakpoint2,type [default: no] 
...
from utils import comp2domins_by_twtest, loadtads, visualization, comp2domins_by_twtest_withSVs
...
if(opts['dvsd']):
    if opts['one']:
        result = comp2domins_by_twtest_withSVs(chrn=opts['<chr>'], start=int(opts['<start>']), end=int(opts['<end>']), reso=int(opts['--reso']), hicnorm=opts['--hicnorm'], fhic0=opts['<hic0>'], fhic1=opts['<hic1>'], min_nbin=int(opts['--min_nbin']), f=opts['--f'],fsvs_condition2 = opts['--fsvs'],SVs_file_condition2 = opts['--svsfile'])
    ...
    if opts['multiple']:
        ...
        def comp2domins_by_twtest_parallel(i):
            tmp_res = comp2domins_by_twtest_withSVs(chrn=tadb.iloc[i, 0], start=tadb.iloc[i, 1], 
